...
| `services` | string[] | `[]` | External services to bring up before `setup:`/`processes:` run. Each name resolves to `runners/lib/services/<name>.sh`, which exports `start_<name>` + `stop_<name>`. Currently supported: `redpanda` (Kafka API at `localhost:9092`, no SASL — needs Docker) and `tigerbeetle` (downloads the latest release binary, formats a single-replica data file, starts on port 3000, symlinks the binary into `$EXAMPLE_DIR/bin/tigerbeetle`). Start order = list order; stop runs on the runner's EXIT trap so transient failures still clean up. |
